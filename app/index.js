...
const workerPath = path.resolve("app", 'factorial-worker.js');
...
const run = async () => {
  const { inputNumber } = await inquirer.prompt([
    {
      type: 'input',
      name: 'inputNumber',
      message: 'Calcular o fatorial de:',
      default: 10,
    },
  ]);
  ...
  console.log(`Diferença entre monothread e com workers: ${Math.floor(diff / 1000000)}ms`);
};
